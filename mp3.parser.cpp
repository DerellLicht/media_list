...
//***************************************************************************************
static unsigned get_id3_size(u8 const * uptr)
{
   u8 p3 = (u8) *uptr++ ;
   u8 p2 = (u8) *uptr++ ;
   u8 p1 = (u8) *uptr++ ;
...
//***************************************************************************************
static u32 get_mp3_header(u8 const * rbfr)
{
   ul2uc_t uconv ;
   uconv.uc[3] = *rbfr++ ;
   uconv.uc[2] = *rbfr++ ;
...
static int parse_mp3_frame(u8 *rbfr, unsigned offset)
{
   ...
   mtemp->mpeg_version = (unsigned int) audio_ver_idx2[mp.h.mpeg_audio_version_id] ;
   mtemp->mpeg_layer = (unsigned int) mpeg_layer_idx[mp.h.layer_desc] ;
...
static int find_mp3_signature(u8 *rbfr, unsigned rlen, int id3_offset)
{
   ...
   if (id3_offset < 0) {
      ...
      for (mp3_offset = id3_offset; mp3_offset < rlen; mp3_offset++) {  //lint !e732
...
static int read_mp3_file(char *fname)
{
   ...
   while (LOOP_FOREVER) {
      ...
      if (first_pass) {
         ...
         mp3_offset = find_mp3_signature(rd_bfr, rdbytes, id3size) ; //lint !e732
         if (mp3_offset < 0) {
